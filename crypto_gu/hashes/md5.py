"""Pure-Python MD5 (RFC 1321).

Not cryptographically secure; kept for legacy protocol work and for the
length-extension attack demonstrations.
"""

from __future__ import annotations

from crypto_gu.encoding import to_bytes

_MASK = 0xFFFFFFFF

_T = (
    0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee,
    0xf57c0faf, 0x4787c62a, 0xa8304613, 0xfd469501,
    0x698098d8, 0x8b44f7af, 0xffff5bb1, 0x895cd7be,
    0x6b901122, 0xfd987193, 0xa679438e, 0x49b40821,
    0xf61e2562, 0xc040b340, 0x265e5a51, 0xe9b6c7aa,
    0xd62f105d, 0x02441453, 0xd8a1e681, 0xe7d3fbc8,
    0x21e1cde6, 0xc33707d6, 0xf4d50d87, 0x455a14ed,
    0xa9e3e905, 0xfcefa3f8, 0x676f02d9, 0x8d2a4c8a,
    0xfffa3942, 0x8771f681, 0x6d9d6122, 0xfde5380c,
    0xa4beea44, 0x4bdecfa9, 0xf6bb4b60, 0xbebfbc70,
    0x289b7ec6, 0xeaa127fa, 0xd4ef3085, 0x04881d05,
    0xd9d4d039, 0xe6db99e5, 0x1fa27cf8, 0xc4ac5665,
    0xf4292244, 0x432aff97, 0xab9423a7, 0xfc93a039,
    0x655b59c3, 0x8f0ccc92, 0xffeff47d, 0x85845dd1,
    0x6fa87e4f, 0xfe2ce6e0, 0xa3014314, 0x4e0811a1,
    0xf7537e82, 0xbd3af235, 0x2ad7d2bb, 0xeb86d391,
)

_S = (
    [7, 12, 17, 22] * 4
    + [5, 9, 14, 20] * 4
    + [4, 11, 16, 23] * 4
    + [6, 10, 15, 21] * 4
)


def _rol(x: int, n: int) -> int:
    return ((x << n) | (x >> (32 - n))) & _MASK


def _compress(state, block):
    m = [int.from_bytes(block[i * 4 : i * 4 + 4], "little") for i in range(16)]
    a, b, c, d = state
    for i in range(64):
        if i < 16:
            f = (b & c) | (~b & d) & _MASK
            g = i
        elif i < 32:
            f = (d & b) | (~d & c) & _MASK
            g = (5 * i + 1) % 16
        elif i < 48:
            f = b ^ c ^ d
            g = (3 * i + 5) % 16
        else:
            f = c ^ (b | (~d & _MASK))
            g = (7 * i) % 16
        f = (f + a + _T[i] + m[g]) & _MASK
        a, d, c = d, c, b
        b = (b + _rol(f, _S[i])) & _MASK
    return ((state[0] + a) & _MASK, (state[1] + b) & _MASK,
            (state[2] + c) & _MASK, (state[3] + d) & _MASK)


def _pad(message: bytes) -> bytes:
    length = len(message) * 8
    padded = message + b"\x80"
    padded += b"\x00" * ((56 - len(padded)) % 64)
    padded += length.to_bytes(8, "little")
    return padded


_INIT = (0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476)


def md5(data) -> bytes:
    """Return the 16-byte MD5 digest of *data*."""
    message = to_bytes(data)
    padded = _pad(message)
    state = _INIT
    for i in range(0, len(padded), 64):
        state = _compress(state, padded[i : i + 64])
    return b"".join(v.to_bytes(4, "little") for v in state)


def md5_hex(data) -> str:
    return md5(data).hex()


def md5_state(data, iv=_INIT):
    """Return the four-word state after processing the padded *data*."""
    padded = _pad(to_bytes(data))
    state = iv
    for i in range(0, len(padded), 64):
        state = _compress(state, padded[i : i + 64])
    return state


__all__ = ["md5", "md5_hex", "md5_state"]
